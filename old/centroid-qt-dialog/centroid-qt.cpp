#include <QDebug>
#include <QFile>
#include <QTextStream>
#include <QRegExp>
#include <QVector>
#include <QDir>
#include <iostream>

#include <QtGui>

#include "ui_particles_form.h"

// for triangulation 
//
#include <vtkVersion.h>
#include <vtkSmartPointer.h>
#include <vtkActor.h>
#include <vtkDelaunay2D.h>
#include <vtkLookupTable.h>
#include <vtkMath.h>
#include <vtkPointData.h>
#include <vtkPoints.h>
#include <vtkPolyData.h>
#include <vtkPolyDataMapper.h>
#include <vtkCellArray.h>
#include <vtkProperty.h>
#include <vtkRenderWindow.h>
#include <vtkRenderWindowInteractor.h>
#include <vtkRenderer.h>
#include <vtkVertexGlyphFilter.h>
#include <vtkXMLPolyDataWriter.h>
//

//////////////////////////////////////////////////////////////////////////

typedef struct 
{
   // unsigned int id; // meaningless
   // position
   float x;
   float y;
   // area
   unsigned int no_pixels;
   // closest particles
   int index_closest_in_next_frame;
   float dist_smallest_next_frame;

   int index_closest_in_previous_frame;
   float dist_smallest_previous_frame;

   bool is_sustained;
} particle;

typedef QVector<particle> particles;
QVector<particles*> list_all_particles;

void read_data();

//////////////////////////////////////////////////////////////////////////

class ParticlesForm : public QWidget, private Ui::ParticlesForm
{
Q_OBJECT

public:
   ParticlesForm(QWidget *parent = 0);

   void read_data();
   void process_particles();
   void process_sustained();
   void process_doubles();

private slots:
   void on_nextButton_clicked();
   void on_previousButton_clicked();
   void on_readDataButton_clicked();
   void on_zoomInButton_clicked();
   void on_zoomOutButton_clicked();
   void on_animateButton_clicked();

   void on_timeScrollBar_valueChanged(int value);
   void on_distanceThresholdDoubleSpinBox_valueChanged(double value);
   void on_particleSizeDoubleSpinBox_valueChanged(double value);
   void on_sustainedSpinBox_valueChanged(int value);
   void on_sustainedCheckBox_stateChanged(int state);

private:
   // void read_data();
   void draw_frame(int frame);

   QGraphicsScene *scene;
   bool isDataRead;
   bool isAnimating;
   QTimer *timer;
   int currentFrame;
   int isSustainedOnlyDisplayed;

   float distance_threshold;
   float particle_size_threshold;
   int particle_time_sustained;

   QImage *image;


   vtkPolyData* outputPolyData;
};

ParticlesForm::ParticlesForm(QWidget *parent)
   : QWidget(parent), 
     isDataRead(false),
     isAnimating(false),
     currentFrame(0),
     isSustainedOnlyDisplayed(0),
     distance_threshold(0.75f),
     particle_size_threshold(50.0f),
     particle_time_sustained(5)
{
   setupUi(this);

   connect(exitButton, SIGNAL(clicked()), qApp, SLOT(quit()));

   timer = new QTimer(parent);
   connect(timer, SIGNAL(timeout()), this, SLOT(on_nextButton_clicked())); 
   timer->stop();

   scene = new QGraphicsScene(this);
   graphicsView->setScene(scene);
   graphicsView->setRenderHint(QPainter::Antialiasing);
   graphicsView->setMouseTracking(true);
   graphicsView->setViewportUpdateMode(QGraphicsView::FullViewportUpdate);
   graphicsView->setDragMode(QGraphicsView::ScrollHandDrag);

   // scene->setForegroundBrush(QColor(255, 255, 255, 127));
   // graphicsView->setStyleSheet("background: transparent");
   // graphicsView->setStyleSheet("background: green");

   timeScrollBar->setRange(0, 0);
   distanceThresholdDoubleSpinBox->setValue(distance_threshold);
   particleSizeDoubleSpinBox->setValue(particle_size_threshold);

   sustainedSpinBox->setValue(particle_time_sustained);

   scene->setSceneRect(0, 0, 600-4, 500-4);

   scene->addText("Particles Viewer"); 
}

void ParticlesForm::on_nextButton_clicked()
{
   ++currentFrame;
   // if (currentFrame < (list_all_particles.size()-1)) ++currentFrame;
   if (currentFrame == list_all_particles.size()) currentFrame = 0;
   draw_frame(currentFrame);
   update();
}

void ParticlesForm::on_previousButton_clicked()
{
   --currentFrame;   
   //if (currentFrame > 0) --currentFrame;
   if (currentFrame < 0) currentFrame = list_all_particles.size()-1;
   draw_frame(currentFrame);
}

void ParticlesForm::on_readDataButton_clicked()
{
   read_data();

   process_particles();
   process_sustained();
   process_doubles();

   draw_frame(0);
}

void ParticlesForm::on_zoomOutButton_clicked()
{
   graphicsView->scale(1.0f/1.2f, 1.0f/1.2f);      
   graphicsView->update();
}

void ParticlesForm::on_zoomInButton_clicked()
{
   graphicsView->scale(1.2f, 1.2f);   
   graphicsView->update();
}

void ParticlesForm::on_timeScrollBar_valueChanged(int value)
{
   draw_frame(value);
}

void ParticlesForm::on_animateButton_clicked()
{
   if (!isAnimating)
   {
      timer->start(100);
   } 
   else 
   {
      timer->stop();
   }
   isAnimating = !isAnimating;
}

void ParticlesForm::on_distanceThresholdDoubleSpinBox_valueChanged(double value)
{
   distance_threshold = value;
   draw_frame(currentFrame);
}

void ParticlesForm::on_particleSizeDoubleSpinBox_valueChanged(double value)
{
   particle_size_threshold = value;
   draw_frame(currentFrame);
}

void ParticlesForm::on_sustainedSpinBox_valueChanged(int value)
{
   particle_time_sustained = value;

   int i, k;

   // reset sustainability
   for (k=0; k<list_all_particles.size(); k++)
   {
      particles *ps0 = list_all_particles[k];
      particle *p = ps0[0].data();
      for (i=0; i<ps0->size(); i++)
      {
         p[i].is_sustained = false;
      }
   }

   // process based on new value
   process_sustained();
   draw_frame(currentFrame);
}

void ParticlesForm::on_sustainedCheckBox_stateChanged(int state)
{
   isSustainedOnlyDisplayed = state;
   draw_frame(currentFrame);
}

#include "centroid-qt.moc"

//////////////////////////////////////////////////////////////////////////

void read_centroid_reg_list(const QString & fileName, particles *ps)
{
   // QFile file("../00002.centroid.reg.list");
   QFile file(fileName);
   if (!file.open(QIODevice::ReadOnly)) 
   {
      qDebug() << file.errorString() << fileName << endl;
   }

   QTextStream in(&file);

   while (!in.atEnd()) 
   {
      QString line = in.readLine();    
      QStringList fields = line.split(QRegExp("\\s+")); 
      particle p;
      // p.id = fields[1].toInt();
      p.no_pixels = fields[2].toInt();
      p.x = fields[3].toFloat();
      p.y = fields[4].toFloat();
      p.index_closest_in_next_frame = -1;
      p.index_closest_in_previous_frame = -1;      
      p.dist_smallest_next_frame = 1000000.0f;
      p.dist_smallest_previous_frame = 1000000.0f;
      p.is_sustained = false;
      ps->push_back(p);
      // qDebug() << fields;
   }   
   std::cout << "#" << std::flush;
}

//////////////////////////////////////////////////////////////////////////

void ParticlesForm::draw_frame(int frame)
{
   scene->clear();
   scene->setSceneRect(0, 0, 600-4, 500-4);


   image = new QImage;
   // image->load("data/00002.centroid.reg.png");
   // image->load("data/00002.centroid.png");
   image->load("data/gray.reg.00002.png");
   QPixmap pixmap = QPixmap::fromImage(*image);
   scene->addPixmap(pixmap); 
   delete(image);
   //
   // graphicsView->setStyleSheet("background-image: url(data/gray.reg.00002.png)");


   if (frame < list_all_particles.size())
   {
      particles *ps = list_all_particles[frame];

      QBrush blackBrush(QColor(0, 0, 0, 127)); //Qt::black,);
      QBrush redBrush(Qt::red);
      QBrush greenBrush(Qt::green);
      QPen blackOutlinePen(QColor(0, 0, 0, 0));
      QPen redOutlinePen(Qt::red);
      QPen greenOutlinePen(Qt::green);
      blackOutlinePen.setWidth(1);
      redOutlinePen.setWidth(1);
      greenOutlinePen.setWidth(1);

      // std::cout << "Number of particles " << ps->size() << " in frame " << frame << std::endl;
      int i;

      // very messy drawing prototype
      //

      if (isSustainedOnlyDisplayed)
      {
         for (i=0; i<ps->size(); i++)
         {
            if (ps->at(i).is_sustained) {

            if ((ps->at(i).dist_smallest_next_frame > distance_threshold) || (ps->at(i).dist_smallest_previous_frame > distance_threshold))
            {
               scene->addEllipse(ps->at(i).x-1, ps->at(i).y-1, 2, 2, blackOutlinePen, blackBrush);
            }
            else
            {
               if (ps->at(i).no_pixels < particle_size_threshold)
               {
                  scene->addEllipse(ps->at(i).x-1, ps->at(i).y-1, 2, 2, redOutlinePen, redBrush);           
               }
               else 
               {
                  scene->addEllipse(ps->at(i).x-1, ps->at(i).y-1, 2, 2, greenOutlinePen, greenBrush);           
               }
            }
            }
         }
      }
      else
      {
         for (i=0; i<ps->size(); i++)
         {

            if ((ps->at(i).dist_smallest_next_frame > distance_threshold) || (ps->at(i).dist_smallest_previous_frame > distance_threshold))
            {
               scene->addEllipse(ps->at(i).x-1, ps->at(i).y-1, 2, 2, blackOutlinePen, blackBrush);
            }
            else
            {
               if (ps->at(i).no_pixels < particle_size_threshold)
               {
                  scene->addEllipse(ps->at(i).x-1, ps->at(i).y-1, 2, 2, redOutlinePen, redBrush);           
               }
               else 
               {
                  scene->addEllipse(ps->at(i).x-1, ps->at(i).y-1, 2, 2, greenOutlinePen, greenBrush);           
               }
            }
         }
      }

      currentFrame = frame;
   }


if (isDataRead)
{
   particles *ps0 = list_all_particles[currentFrame];

   // Create a grid of points (height/terrian map)
   vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();

   double xx, yy, zz;
   for (int i=0; i<ps0->size(); i++)
   {
      xx = ps0->at(i).x;
      yy = ps0->at(i).y;
      zz = vtkMath::Random(-.5, .5);
      points->InsertNextPoint(xx, yy, zz);
   }

   // Add the grid points to a polydata object
   vtkSmartPointer<vtkPolyData> inputPolyData = vtkSmartPointer<vtkPolyData>::New();
   inputPolyData->SetPoints(points);
 
   // Triangulate the grid points
   vtkSmartPointer<vtkDelaunay2D> delaunay = vtkSmartPointer<vtkDelaunay2D>::New();
   
   #if VTK_MAJOR_VERSION <= 5
      delaunay->SetInput(inputPolyData);
   #else
      delaunay->SetInputData(inputPolyData);
   #endif
   delaunay->Update();

   vtkPolyData* outputPolyData = delaunay->GetOutput();

   std::cout << "Number of polys: " << outputPolyData->GetNumberOfPolys() << std::endl;

   vtkIdType npts, *pts;
   outputPolyData->GetPolys()->InitTraversal();
   
   while (outputPolyData->GetPolys()->GetNextCell(npts, pts))
   {
      // std::cout << "triangle [" << pts[0] << " " << pts[1] << " " << pts[2] << "]" << std::endl;

      double p0[3];
      outputPolyData->GetPoint(pts[0], p0);

      double p1[3];
      outputPolyData->GetPoint(pts[1], p1);

      double p2[3];
      outputPolyData->GetPoint(pts[2], p2);
      
      QPen redOutlinePen(Qt::red);
      redOutlinePen.setWidth(0.1);

      scene->addLine(p0[0], p0[1], p1[0], p1[1], redOutlinePen);
      scene->addLine(p1[0], p1[1], p2[0], p2[1], redOutlinePen);
      scene->addLine(p2[0], p2[1], p0[0], p0[1], redOutlinePen);
   }


}

   /*
   QLineF topLine(scene->sceneRect().topLeft(), scene->sceneRect().topRight());
   QLineF leftLine(scene->sceneRect().topLeft(), scene->sceneRect().bottomLeft());
   QLineF rightLine(scene->sceneRect().topRight(), scene->sceneRect().bottomRight());
   QLineF bottomLine(scene->sceneRect().bottomLeft(), scene->sceneRect().bottomRight());
   QPen myPen = QPen(Qt::red);
   scene->addLine(topLine, myPen);
   scene->addLine(leftLine, myPen);
   scene->addLine(rightLine, myPen);
   scene->addLine(bottomLine, myPen);
   */

   timeScrollBar->setValue(frame);

   QString lf = "Frame: ";
   lf.append(QString::number(currentFrame));
   labelCurrentFrame->setText(lf); 
   graphicsView->update();
}

//////////////////////////////////////////////////////////////////////////

void ParticlesForm::read_data()
{
   QDir dir;
   dir.setFilter(QDir::Files | QDir::Hidden | QDir::NoSymLinks);
   dir.setSorting(QDir::Name);

   dir.setPath("./data");
   //dir.setNameFilters(QStringList("?????.centroid.reg.list")); 
   dir.setNameFilters(QStringList("0000?.centroid.reg.list")); 

   QFileInfoList list = dir.entryInfoList();
   std::cout << "Number of files: " << list.size() << std::endl;

   list_all_particles.resize(list.size());

   if (list.size()>0)
      timeScrollBar->setRange(0, list.size()-1);

   std::cout << "Reading data:" << std::endl;

   for (int i=0; i<list.size(); ++i) 
   {
      list_all_particles[i] = new particles;

      QFileInfo fileInfo = list.at(i);
      read_centroid_reg_list(QString("%1").arg(fileInfo.absoluteFilePath()), list_all_particles[i]);
      // std::cout << qPrintable(QString("%1").arg(fileInfo.fileName())) << std::endl;
   }   
   std::cout << std::endl << std::flush;

   if (list_all_particles.size() > 0) isDataRead = true;

   draw_frame(0);
}

//////////////////////////////////////////////////////////////////////////

void ParticlesForm::process_particles()
{
   int i, j, k, m;

   for (k=0; k<list_all_particles.size()-1; k++)
   {
      //
      //
      // finds particle index and closest distance to particles (from next frame)

      particles *ps0 = list_all_particles[k];
      particles *ps1 = list_all_particles[k+1];

      for (i=0; i<ps0->size(); i++)
      {
         float x0 = ps0->at(i).x; 
         float y0 = ps0->at(i).y;
         float dist0 = ps0->at(i).dist_smallest_next_frame;
         int j0 = -1;

         for (j=0; j<ps1->size(); j++)
         {
            float x1 = ps1->at(j).x; 
            float y1 = ps1->at(j).y;

            float dx = x1-x0;
            float dy = y1-y0;

            float dist = sqrt(dx*dx + dy*dy);

            if (dist < dist0)
            {
               dist0 = dist;
               j0 = j;
            }
         }

         particle *p = ps0[0].data();
         p[i].dist_smallest_next_frame = dist0;
         p[i].index_closest_in_next_frame = j0;
      }

      // 
      //
      // finds particle index and closest distance to particles (from previous frame)


      m = k+1;

      ps0 = list_all_particles[m];
      ps1 = list_all_particles[m-1];

      for (i=0; i<ps0->size(); i++)
      {
         float x0 = ps0->at(i).x; 
         float y0 = ps0->at(i).y;
         float dist0 = ps0->at(i).dist_smallest_previous_frame;
         int j0 = -1;

         for (j=0; j<ps1->size(); j++)
         {
            float x1 = ps1->at(j).x; 
            float y1 = ps1->at(j).y;

            float dx = x1-x0;
            float dy = y1-y0;

            float dist = sqrt(dx*dx + dy*dy);

            if (dist < dist0)
            {
               dist0 = dist;
               j0 = j;
            }
         }

         particle *p = ps0[0].data();
         p[i].dist_smallest_previous_frame = dist0;
         p[i].index_closest_in_previous_frame = j0; 
      }

      //
      //
   }
}

//////////////////////////////////////////////////////////////////////////

void ParticlesForm::process_sustained()
{
   int k, i, j;

   if ((list_all_particles.size()-particle_time_sustained) > 0)
   {
   //
   for (k=0; k<list_all_particles.size()-particle_time_sustained; k++)
   {
      particles *ps0 = list_all_particles[k];

      for (i=0; i<ps0->size(); i++)
      {
         int isSustained = 0;
         int index_next = ps0->at(i).index_closest_in_next_frame;

         if (ps0->at(i).dist_smallest_next_frame < distance_threshold)
         {
            isSustained = 1;
            for (j=0; j<particle_time_sustained-1; j++)
            {
               particles *ps1 = list_all_particles[k+j+1];
               if (ps1->at(index_next).dist_smallest_next_frame < distance_threshold)
               {
                  isSustained++;
               }
               int ii = index_next;
               index_next = ps1->at(ii).index_closest_in_next_frame;
            }
         }
         if (isSustained == particle_time_sustained) 
         {
            particle *p = ps0[0].data();
            p[i].is_sustained = true;
         }
      }
   }
   //
   }
}

//////////////////////////////////////////////////////////////////////////

void ParticlesForm::process_doubles()
{
   int i, j, k;

   for (k=0; k<list_all_particles.size(); k++)
   {
      particles *ps0 = list_all_particles[k];

      for (i=0; i<ps0->size(); i++)
      {
         int index = ps0->at(i).index_closest_in_next_frame;
         int count = 0;
         for (j=0; j<ps0->size(); j++)
         {
            if (index == ps0->at(j).index_closest_in_next_frame) count++;
         }
         // std::cout << count << " ";
      }
      // std::cout << std::endl;
   }
}


//////////////////////////////////////////////////////////////////////////

int main(int argc, char *argv[])  
{
   QApplication app(argc, argv);

   ParticlesForm pf;
   // pf.read_data();
   pf.show();

   /*
   std::cout << list_all_particles.size() << std::endl;
   for (int i=0; i<list_all_particles.size(); i++)
   {
      std::cout << list_all_particles[i]->size() << "  ";
   }
   */
   // process();

   return app.exec();
}

//////////////////////////////////////////////////////////////////////////
